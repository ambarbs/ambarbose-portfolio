export const hero = {
  name: "Ambar Bose",
  title: "Senior Software Engineer",
  subtitle: "Web Platforms · Full-Stack Delivery · AI-Enabled Workflows",
  location: "Sydney, Australia",
  summary:
    "I build scalable web platforms, internal tools, and AI-assisted workflows across React, Next.js, TypeScript, Node.js, REST/GraphQL APIs, CI/CD and cloud-hosted delivery.",
  links: {
    cv: "/ambar-bose-cv.pdf",
    projects: "#projects",
    linkedin: "https://www.linkedin.com/in/ambarbose/",
    github: "https://github.com/ambarbs",
  },
};

export const heroTechBadges = ["React", "Next.js", "TypeScript", "Node.js", "AWS"];

export const about =
  "Ambar is a Sydney-based Senior Software Engineer with 10+ years of Australian delivery experience across enterprise web platforms, healthcare, energy, insurance, media, travel, telecom and financial services. He has a strong background in TypeScript, React, Next.js, Angular, Node.js, REST/GraphQL APIs, CI/CD and cloud-hosted delivery, with experience across CMS/schema-driven UI, micro-frontends, .NET/Azure-backed service integration and AI-enabled workflows using OpenAI APIs.";

export const experienceGroups = [
  {
    title: "Featured Experience",
    featured: true,
    roles: [
      {
        company: "Vitura Health",
        role: "Senior Software Engineer",
        period: "Oct 2025",
        location: "Sydney / Remote",
        summary:
          "Healthcare platform delivery across modern web, API-backed workflows, and AI-enabled product thinking.",
        tags: ["TypeScript", "React", "Next.js", "Node.js", "APIs", "AI"],
        techStack:
          "TypeScript, React, Next.js, Node.js, REST APIs, AI-enabled workflows, cloud-hosted delivery",
        bullets: [
          "Building healthcare web platform workflows with TypeScript, React/Next.js patterns, and pragmatic full-stack delivery.",
          "Supporting API-backed product experiences with a focus on maintainable implementation and clear user journeys.",
          "Applying AI-enabled workflow thinking to practical healthcare product problems.",
        ],
      },
      {
        company: "Ampol Australia",
        role: "Senior Software Engineer - Contract",
        period: "May 2022 to Sep 2025",
        location: "Sydney",
        summary:
          "Enterprise energy platform work spanning schema-driven UI, reusable components, APIs, and CI/CD.",
        tags: ["React", "Next.js", "TypeScript", "CMS", "GraphQL", "CI/CD"],
        techStack:
          "React, Next.js, TypeScript, CMS/schema-driven UI, component registries, REST/GraphQL APIs, CI/CD, cloud-hosted delivery",
        bullets: [
          "Delivered enterprise web platform features across React, Next.js, TypeScript, CMS/schema-driven UI, and reusable component patterns.",
          "Integrated frontend journeys with REST/GraphQL APIs, CI/CD pipelines, and cloud-hosted delivery practices.",
          "Worked across reusable UI/API patterns to keep product delivery consistent across teams and workflows.",
        ],
      },
      {
        company: "IAG",
        role: "Senior Software Engineer - Contract",
        period: "Mar 2021 to May 2022",
        location: "Sydney",
        summary:
          "Insurance platform delivery across dynamic forms, multi-step journeys, and regulated service integration.",
        tags: ["React", "TypeScript", "Forms", "APIs", "Azure", "CI/CD"],
        techStack:
          "TypeScript, React, dynamic forms, multi-step workflows, REST APIs, .NET/Azure-backed integration, CI/CD",
        bullets: [
          "Built insurance platform experiences using modern frontend architecture, dynamic forms, and multi-step customer workflows.",
          "Worked across reusable UI/API patterns and service integration in a regulated enterprise delivery environment.",
          "Helped maintain clean, testable implementation patterns across complex customer journeys.",
        ],
      },
      {
        company: "Optus Sport",
        role: "Full Stack Developer - Contract",
        period: "Feb 2019 to Feb 2020",
        location: "Sydney",
        summary:
          "Media product delivery across customer-facing subscription experiences, frontend work, and backend APIs.",
        tags: ["React", "Node.js", "APIs", "Media", "Full-stack", "Delivery"],
        techStack:
          "React, Node.js, REST APIs, full-stack delivery, customer-facing media workflows, production release practices",
        bullets: [
          "Contributed to media and subscription product experiences across frontend and backend delivery.",
          "Supported high-traffic customer-facing workflows with React, Node.js, APIs, and production release practices.",
          "Worked in a product environment where performance, release quality, and user experience were central.",
        ],
      },
    ],
  },
  {
    title: "Additional Australian Experience",
    featured: false,
    roles: [
      {
        company: "Luxury Escapes",
        role: "Senior Software Engineer - Contract",
        period: "Oct 2020 to Mar 2021",
        location: "Sydney",
        summary:
          "Travel product delivery across modern web interfaces, APIs, and customer-facing booking workflows.",
        tags: ["React", "TypeScript", "APIs", "Travel"],
        techStack:
          "React, TypeScript, JavaScript, REST APIs, customer-facing travel and booking workflows",
        bullets: [
          "Delivered customer-facing travel product features with a focus on clean web implementation.",
          "Worked across frontend workflows and API integration in a fast-moving product team.",
        ],
      },
      {
        company: "RapiScan Systems",
        role: "Senior Software Engineer",
        period: "Aug 2020 to Oct 2020",
        location: "Sydney",
        summary:
          "Engineering delivery for specialist systems with pragmatic frontend and integration work.",
        tags: ["TypeScript", "JavaScript", "APIs", "Delivery"],
        techStack:
          "TypeScript, JavaScript, frontend engineering, API integration, product delivery",
        bullets: [
          "Contributed to specialist software delivery across frontend and integration tasks.",
          "Helped maintain clear implementation patterns across a short, focused engagement.",
        ],
      },
      {
        company: "Suncorp",
        role: "Senior Software Consultant - Contract",
        period: "Mar 2020 to Jul 2020",
        location: "Sydney",
        summary:
          "Insurance and financial services delivery across frontend workflows and enterprise integration.",
        tags: ["React", "TypeScript", "APIs", "Insurance"],
        techStack:
          "React, TypeScript, JavaScript, REST APIs, enterprise integration, insurance workflows",
        bullets: [
          "Delivered web application features in an enterprise insurance and financial services setting.",
          "Worked across frontend implementation and service integration with attention to maintainability.",
        ],
      },
      {
        company: "CoreLogic",
        role: "Full Stack Software Engineer - Contract",
        period: "Sep 2018 to Feb 2019",
        location: "Sydney",
        summary:
          "Full-stack product work across property data experiences, APIs, and web application delivery.",
        tags: ["Full-stack", "React", "Node.js", "APIs"],
        techStack:
          "React, JavaScript, Node.js, REST APIs, full-stack web delivery, data-backed product workflows",
        bullets: [
          "Built full-stack web features for data-backed product experiences.",
          "Worked across frontend implementation, backend integration, and production delivery.",
        ],
      },
      {
        company: "Tabcorp",
        role: "Senior Software Developer - Contract",
        period: "Mar 2018 to Sep 2018",
        location: "Sydney",
        summary:
          "Financial services product delivery across frontend features, API integration, and production systems.",
        tags: ["JavaScript", "APIs", "Frontend", "Delivery"],
        techStack:
          "JavaScript, frontend engineering, REST APIs, production systems, financial services delivery",
        bullets: [
          "Delivered product features in a fast-moving financial services environment with a focus on quality and maintainability.",
          "Worked across frontend implementation, API integration, and team delivery practices for production systems.",
        ],
      },
      {
        company: "NBN",
        role: "Senior Solution Developer - Contract",
        period: "Jul 2017 to Mar 2018",
        location: "Sydney",
        summary:
          "Telecom solution delivery across enterprise systems, integration work, and reusable delivery patterns.",
        tags: ["Enterprise", "Integration", "APIs", "Telecom"],
        techStack:
          "Enterprise web delivery, service integration, REST APIs, reusable implementation patterns, telecom workflows",
        bullets: [
          "Contributed to enterprise telecom solution delivery across integration and web application work.",
          "Supported reusable implementation patterns for complex business workflows.",
        ],
      },
      {
        company: "IRESS",
        role: "Full Stack Software Developer",
        period: "Sep 2015 to Jul 2017",
        location: "Sydney",
        summary:
          "Full-stack financial services software delivery across web interfaces, APIs, and backend integration.",
        tags: ["Full-stack", "JavaScript", "APIs", "Finance"],
        techStack:
          "JavaScript, full-stack web development, APIs, backend integration, financial services software",
        bullets: [
          "Built full-stack financial services software across web interfaces, APIs, and backend integration work.",
          "Contributed to reusable engineering patterns and production delivery across established product teams.",
        ],
      },
    ],
  },
];

export const earlierGlobalExperience = [
  "Oracle — Senior Application Developer",
  "Cisco Systems — Software Engineer II",
  "Yodlee — Software Engineer",
  "Indience Info Systems / Unvired — Senior Software Engineer",
];

export const projects = [
  {
    name: "AI Messaging Copilot",
    description:
      "Chrome extension + Node.js backend + OpenAI APIs. User-in-the-loop assistant that reads selected conversation context and generates tone-based reply suggestions.",
    image: "/projects/ai-messaging-copilot.png",
    tags: ["Chrome Extension", "Node.js", "OpenAI APIs"],
  },
  {
    name: "AI Boardroom",
    description:
      "Multi-agent AI discussion app using OpenAI, Gemini, Claude, Supabase and Vercel.",
    image: "/projects/ai-boardroom.png",
    tags: ["Multi-agent AI", "Supabase", "Vercel"],
  },
  {
    name: "Our Life",
    description:
      "Private shared memory and journal app using Next.js and Supabase.",
    image: "/projects/our-life.png",
    tags: ["Next.js", "Supabase", "Private App"],
  },
  {
    name: "RoleRadar",
    description: "AI-assisted job tracking and role-matching concept.",
    image: "/projects/roleradar.png",
    tags: ["AI Matching", "Job Tracking", "Product Concept"],
  },
];

export const techStack = [
  {
    group: "Frontend & Web",
    items: [
      "TypeScript",
      "JavaScript",
      "React",
      "Next.js",
      "Angular",
      "Redux",
      "MobX",
      "React Native",
      "HTML5",
      "CSS3/SASS",
      "Tailwind",
    ],
  },
  {
    group: "Backend & Integration",
    items: [
      "Node.js",
      "REST APIs",
      "GraphQL",
      ".NET/Azure integration",
      "Java/Spring exposure",
      "Python exposure",
    ],
  },
  {
    group: "Architecture",
    items: [
      "CMS/schema-driven UI",
      "Component registries",
      "Dynamic forms",
      "Multi-step workflows",
      "Micro-frontends",
      "Reusable UI/API patterns",
    ],
  },
  {
    group: "Cloud, DevOps & Quality",
    items: [
      "AWS Lambda",
      "API Gateway",
      "AppSync",
      "S3",
      "Azure SWA",
      "CI/CD",
      "Jest",
      "React Testing Library",
      "Selenium",
    ],
  },
];
